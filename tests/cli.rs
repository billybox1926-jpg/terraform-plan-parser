...
fn filters_only_update_actions() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_terraform_plan_parser"))
        ...
        .arg("--format").arg("csv").arg("--include-action").arg("update")
        .env("PATH", "").output().expect("run terraform_plan_parser");
    assert!(output.status.success(), "stderr: {}", String::from_utf8_lossy(&output.stderr));
    assert_eq!(String::from_utf8_lossy(&output.stdout), "resource_type,resource_name,action\naws_s3_bucket,logs,update\n");
    fs::remove_dir_all(root).expect("remove temp dir");
}
...
fn filters_only_delete_actions() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_terraform_plan_parser"))
        .arg(".").current_dir(&root).arg("--plan-file").arg("plan.ndjson")
        .arg("--format").arg("csv").arg("--include-action").arg("delete")
        .env("PATH", "").output().expect("run terraform_plan_parser");
    assert!(output.status.success(), "stderr: {}", String::from_utf8_lossy(&output.stderr));
...
fn excludes_actions_even_when_included() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_terraform_plan_parser"))
        .arg(".").current_dir(&root).arg("--plan-file").arg("plan.ndjson")
        .arg("--format").arg("csv")
        .arg("--include-action").arg("create,update,delete")
        .arg("--exclude-action").arg("delete")
        .env("PATH", "").output().expect("run terraform_plan_parser");
    assert!(output.status.success(), "stderr: {}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
